...
class DownloadAndLoadMochiModel:
    ...
    def loadmodel(self, model, vae, precision, attention_mode, trigger=None, compile_args=None, cublas_ops=False, rms_norm_func="default"):
        ...
        if (not os.path.exists(model_path)) and os.path.exists(os.path.join(folder_paths.cache_dir, 'diffusion_models', 'mochi', model)):
            model_path = os.path.join(folder_paths.cache_dir, 'diffusion_models', 'mochi', model)
        ...
        if (not os.path.exists(vae_path)) and os.path.exists(os.path.join(folder_paths.cache_dir, 'VAE', 'mochi', vae)):
            vae_path = os.path.join(folder_paths.cache_dir, 'VAE', 'mochi', vae)
